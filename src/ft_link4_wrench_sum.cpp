...
#include "link4_wrench_sum_assistance.hpp"
...
int wam_main(int, char**, barrett::ProductManager&, barrett::systems::Wam<DOF>&) {
    std::cerr << "Wrench-sum assistance requires a 7-DOF WAM.\n";
    return 1;
}
...
int wam_main<7>(int argc, char** argv, barrett::ProductManager& pm,
    ...
    const auto settings = link4_wrench_sum::loadSettings(LINK4_WRENCH_SUM_CONFIG_PATH);
    ...
    Link4WrenchSumAssistance assistance(mounting, settings, pm.getExecutionManager()->getPeriod());
    ...
    // Temporarily disable continuous diagnostics so enable/fault messages stay visible.
    ...
    std::cout << "Sensor torque - end-effector-remapped torque: gain=" << settings.gain << ", limit=" << settings.torqueLimit << " Nm/joint.\n"
    ...
    while (true) {
        if (enabled && assistance.fault()) {
            wam.idle();
            enabled = false;
            std::cout << "Wrench-sum assistance DISABLED: fault " << assistance.fault()
                      << " (1=invalid input, 2=singular Jacobian or invalid mapping, 3=invalid limiter input/output).\n"
        ...
        enabled = !enabled;
        std::cout << "Wrench-sum assistance " << (enabled ? "enabled" : "disabled") << ".\n";
    }
    wam.idle();
    wam.gravityCompensate();
